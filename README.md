...
이 프로젝트를 통해 **흩어진 데이터를 현업의 판단 과정에 맞는 분석 제품으로 연결하는 방법**을 경험했습니다.

| 조직의 과제 | 제공할 수 있는 변화 |
|---|---|
| 반복적으로 작성하는 성과 보고 | 자동 갱신되는 데이터 파이프라인과 KPI 구조 |
| 분리된 고객·행동·매출 데이터 | 고객 맥락이 포함된 마케팅 분석 모델 |
| 해석하기 어려운 분석 결과 | 현업이 바로 사용할 수 있는 대시보드 |
| 근거가 불분명한 생성형 AI 답변 | 계산과 해석을 분리한 검증 가능한 AI 기능 |
| 실행 후 효과를 확인하기 어려운 제안 | 액션과 확인 KPI가 연결된 실험 구조 |

> **데이터를 보여주는 것에서 끝내지 않고, 현업이 다음 행동을 결정하고 그 결과를 검증할 수 있는 구조를 만듭니다.**
