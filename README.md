...
이 프로젝트는 BigQuery, Cloud Run Jobs, Cloud Scheduler를 활용해 데이터 갱신과 인사이트 생성을 자동화했습니다.
...
1. 이벤트 KPI 테이블 갱신
2. 이벤트별 순위, 사분면, 추천 액션 계산
3. 이벤트별 AI 인사이트 생성
4. BigQuery 결과 테이블 저장
5. Looker Studio 대시보드 갱신

일별 자동화 대상 테이블은 다음과 같습니다.

| 테이블 | 역할 |
|---|---|
| `KPI_EVENT_TEST` | 이벤트별 KPI와 분석 지표 저장 |
| `KPI_EVENT_TEST_AI_INSIGHTS` | 이벤트별 AI 인사이트 저장 |
...
1. 최근 3개월 유저 행동 데이터 집계
2. 유저 군집 분석
3. 군집별 이름 생성
4. 유저별 군집 정보를 BigQuery에 저장
5. 일별 이벤트 KPI 생성 시 군집 정보 반영

분기별 자동화 대상 테이블은 다음과 같습니다.

| 테이블 | 역할 |
|---|---|
| `KPI_USER_CLASS` | 유저별 군집 번호와 군집명 저장 |

---

### 7.3 Cloud Run Jobs 구성

자동화 작업은 실행 목적에 따라 Cloud Run Jobs로 분리했습니다.

| Job | 실행 역할 |
|---|---|
| `kpi-user-class` | 분기별 유저 군집 생성 |
| `kpi-event` | 일별 이벤트 KPI 생성 |
| `kpi-event-insights` | 일별 AI 인사이트 생성 |

작업을 분리해 각 단계의 실행 주기를 다르게 설정하고, 실패 시 필요한 작업만 재실행할 수 있도록 구성했습니다.
